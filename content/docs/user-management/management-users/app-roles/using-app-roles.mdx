...
The permissions list displays to which roles a permission is needed. This helps you identify if a permission is still needed after removing a role.
...
In the `App Roles` section, a warning icon will be displayed if the user is missing a required permission for a role. Clicking on the icon to display a list of missing permissions.
An practical way to fix this is often to remove an the app role and assign it again.
