...
Permissions define which actions the service account is allowed to perform in the system. This follows the principle of least privilege - only grant the permissions that are absolutely necessary.
...
1. **Resource Type**: Select the type of resource (e.g., "Study")
2. **Resource Key**: Specify the specific resource identifier (e.g., study key like "tekenradar") or use `*` for all resources of that type
3. **Action**: Select the action the service account is allowed to perform (e.g., "get-reports")
4. **Limiter** (optional): Define additional constraints in JSON format. For example, to limit access to specific reports, use the format `[{"reportKey": "report1"}]`
