...
The study engine is the core component of the study running on the server. It's main responsibility is to handle study events based on each study's own rule set.
...
These rules encode the study flow. The engine supports a large set of actions and expressions, providing high level of freedom in the study design.
...
- *Condition:* The participant's age is less than 18. -> expression to compare a response value with the current date
- *Action:* Set the participant's age category to *child*. -> action to set a participant flag
