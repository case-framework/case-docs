...
* **How it works:** A date is chosen via the picker and stored as a date value.  
...
#### 3. **Get event payload value as number**  
...
* **How it works:** The key of the event payload is accessed, and the specified value is parsed into a number.  
...
* **Meaning:** Converts a given value (e.g., a string or response) into a numeric value.  
* **Use Case:** Helpful when participant input needs to be treated as a number for calculations or comparisons.  
* **How it works:** Enter the value or response key; the expression attempts to parse it into a number.  

### Advanced

#### 3. **Use external service to evaluate event data (return float)**  
...
* **How it works:** Event data is passed to the configured external service, and the numeric result is returned.  
...
* **How it works:** Define the range (min/max), and the expression outputs a random number in that range.  

### Date helpers

#### 5. **Get ISO week for timestamp**  
...
* **How it works:** Provide a timestamp, and the expression calculates its ISO week number.  

#### 1. **Get timestamp for next occurrence of an ISO week**  
...
#### 2. **Get timestamp for next occurrence of a start of month**  
...
* **How it works:** Accesses the response by key and parses it into a number.
...
* **How it works:** Evaluates the response array and returns the count of selected items.


### Participant State Checkers

#### 1. **Get study entry time**  
* **Meaning:** Returns the timestamp of when the participant entered the study.  
* **Use Case:** Useful for calculating durations, deadlines, or eligibility based on study entry.  
* **How it works:** Retrieves the stored entry time for the participant and returns it as a timestamp.
