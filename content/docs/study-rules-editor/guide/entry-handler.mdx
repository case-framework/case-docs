...
<Steps> 
<Step> 
In your study rules dahboard click on the section **Entry event handler**. 
</Step> <Step> 
In the ENTRY hanlder editor, click **"+ add value"**. From the list of available actions, select **"Schedule a message"**. 
</Step> <Step> 
Choose the message type *"Welcome"* you defined in your survey context. 
 ...
 Add another action by clicking **+ add value** and select **Add new survey**. 
  ...
  Select the survey key *"intake"*. Keep **Available from** set to *0 days* (current timestamp) and leave **Available until** empty if the survey should remain available. 
</Step> <Step> 
  Now, add a third action: click **+ Add action** and select **Schedule a message**. 
</Step> <Step> 
  Choose the message *"Reminder"*. 
</Step> <Step> 
  Under **Scheduled for**, set an offset such as *3 days*. This ensures the reminder message is sent 3 days after entry. 
</Step> <Step> 
  Your ENTRY event will now automatically: 1. Send a welcome message immediately. 2. Assign the intake survey. 3. Schedule a reminder message after the defined delay. 
</Step> </Steps>